...
import { getUserDidFragment, getHashFragment } from '../libs/user';
...
const news_type_default = 'articles';
  ...
  render() {
    ...
    return (
      <Tooltip trigger={['focus']} title={title} placement="topLeft" overlayClassName="numeric-input">
        <Input {...this.props} onChange={this.onChange} onBlur={this.onBlur} placeholder="如: 0.008" maxLength={8} />
      </Tooltip>
    );
  }
}
...
const renderPaytipList = (x, token) => (
  <span className="antd-list-comment-list-item-text">
    ...
    {(x.tx_hash && x.tx_hash.length>0) ? 
        <a href={x.tx_hash_link} target="_blank" style={{ fontSize: '12px', color: '#888888' }}> - 哈希{getHashFragment(x.tx_hash)}@{x.time}</a>
        :
...
class App extends Component {
  ...
  updateToPayValue = () => {
    const { news_type, news_title_enabled, news_title_to_send, news_content_to_send, news_to_send_weight, news_image_url } = this.state;
    ...
    if (news_type != 'test2' && news_type != 'articles_1' && newsLength > 0) {
  ...
  onNewsImageUploadChange = ({ fileList }) => {
    ...
    for (var i = 0; i < fileList.length; i++) {
      ...
      if (!isJpgOrPng) {
        ...
        if (!isLt5M) {
          Modal.error({ title: '图片大小需小于5MB!', maskClosable: 'true' });
  ...
  onCommentFocus = ({ }) => {
    ...
    this.setState({ comment_to_send: ['朕已阅', '鉴定完毕，不明觉厉', 'abt加油⛽️  ', '今天发的晚，差评！', '可以但没有必要', 'abt蒸蒸日上', 'abt换新天', '你说的都对', '你长的丑，你说的都对', 'you see see you one day day de', 'you can you up', '老娘天下最美', '你过来我保证不打死你', '画面太美，不敢想象', '我只想安静地做个美男子', '我读书少你别骗我', '我太南了', '感觉人生已经到达了巅峰', '我不要你觉得，我要我觉得', '你爱我吗？你变了！', 'DUCK不必', '求锤得锤', 'OMG，买它！', '原谅我不厚道地笑了', '人间不值得', '陈独秀同学让一让', '话不多说，直接上图', '瀑布算个球，富贵险中求', '评论一时爽，一直评论一直爽', '我40米长的大砍刀呢？', '我就笑笑，不说话', '我就静静地看你装逼', '1冒币1别野', '为梦想打钱', '不要怂，就是干，全梭。赢了会所嫩模，输了下海干活，大不了再打十年工', '气质这块拿捏的死死的', '天王盖地虎！', '宝塔镇河妖！', '好看的皮囊千篇一律，有趣的灵魂万里挑一', '好看的皮囊三千一晚；有趣的灵魂都在梭哈', ' O_o π_π T^T ⊙﹏⊙ ^O^ O(∩_∩)O', '咱也不知道，咱也不敢问', '此图币🔥', '啤酒🍺饮料矿泉水，花生瓜子八宝粥', '未来已来', '臣妾做不到啊做不到', '朕知道了，你们都退下'].randomElement() });
  };
  ...
  /*Send news handler*/
  handleSendNews = () => {
    console.log('handleSendNews start');
    ...
    if (true) {
      const asset_did = news_content_to_send.length > 0 ? HashString('sha1', user.did+Date.now()+news_content_to_send) : HashString('sha1', user.did+Date.now()+news_image_url);
      ...
      if (news_to_chain_mode === 'direct') {
        ...
        if (news_type === 'test2' || news_type === 'articles_1') {
  ...
  onListItemActionClick = async (action_type, asset_did) => {
    ...
    console.log('newsflashItem:', newsflashItem)
  ...
  onPaytipValueFocus = ({ }) => {
    ...
    this.setState({ paytip_input_value: [0.0006, 0.0008, 0.0088, 0.0888, 0.888, 0.2333, 0.666, 0.1314, 0.1024, 0.186, 0.9999].randomElement() });
    this.setState({ paytip_comment: ['不能同意更多', '一看你就很有思想', '你真有品味', '爱你哦，么么哒', '爱的供养'].randomElement() });
  };
  ...
  onPaytipCommentFocus = ({ }) => {
    Array.prototype.randomElement = function () {
        return this[Math.floor(Math.random() * this.length)]
    }

    this.setState({ paytip_comment: ['不能同意更多', '一看你就很有理想', '你真有品味', 'up主太帅啦', '爱你哦，么么哒', '爱的供养'].randomElement() });
  };
  ...
  render() {
    ...
    const newsImageUploadButton = (
      <div>
        <Icon type="plus" />
        <div className="ant-upload-text">小于5M<br />（可选）</div>
      </div>
    );
      ...
      <Layout title={env.appName}>
        <Main>
          ...
          <Menu onClick={this.handleNewsTypeMenuChange} selectedKeys={news_type} mode="horizontal">
            <Menu.Item key="hot">
              <span style={{ fontSize: '15px', fontWeight: 600 }}>最热</span>
            </Menu.Item>
            <Menu.Item key="articles">
              <span style={{ fontSize: '15px' }}>最新</span>
            </Menu.Item>
          ...
          <LocaleProvider locale={zh_CN}>
                  ...
                  renderItem={item => (
                    <List.Item
                      ...
                      <div>
                        ...
                        {(item.news_type === 'test2' || item.news_type === 'articles') && item.news_article_worth > 0 && (
                          <span style={{ fontSize: '10px', color: '#FF0000', marginRight: 0 }}>
                            {' '}
                        ...
                        <a
                          ...
                          target="_blank"
                          style={{ fontSize: '12px', color: '#7B68EE' }}>
                          {item.news_create_from_name}
                        </a>{' '}
                        <br />
                        <a href={'https://gateway.ipfs.io/ipfs/'+item.news_images[3]} target="_blank" style={{ fontSize: '11px', color: '#000000' }}>
                        IPFS
                        </a>{' - '}
                      ...
                      {(item.news_type === 'test2' || item.news_type === 'articles') && (
                        <div
                          ...
                          <span style={{ width: '100%' }}>
                            ...
                            {item.news_title > 0 ? '<br />' : ''}
                            <span style={{ fontSize: '16px', fontWeight: 300, color: '#000000' }}>
                              <AutoLinkText text={item.news_content} linkProps={{ target: '_blank' }} />
                            </span>{' '}
                            <br />
                            {item.news_images[2] || item.news_images[1] ? (<a href={item.news_images[2] ? '/ipfs/'+item.news_images[2] : item.news_images[1]} ><img
                              src={item.news_images[2] ? '/ipfs/'+item.news_images[2] : item.news_images[1]}
                              alt="HashNews"
                              width="50%"
                              //height="210"
            ...
            <Modal
              //title={this.state.news_type === 'articles' ? '图文' : '快讯'}
              closable={false}
              ...
              <div style={{ margin: '10px 0' }}>
                <Text style={{ fontSize: '15px', color: '#000000', marginRight: 10 }}>来源</Text>
                <Select
              ...
              {(news_type === 'test2' || news_type === 'articles_1') && (
                <div style={{ margin: '10px 0' }}>
                  <div style={{ float: 'left', marginTop: 3, marginRight: 10 }}>
                    <Text style={{ fontSize: '15px', color: '#000000' }}>定价</Text>
                  </div>
                  <div>
                    <NumericInput
                      style={{ width: 120 }}
              ...
              {news_type == 'chains' && (
                ...
                <div style={{ margin: '10px 0' }}>
                  <span style={{ fontSize: '15px', color: '#000000', marginRight: 10 }}>标题</span>
                  <Checkbox
              ...
              <div style={{ margin: '5px 0' }}>
                <TextArea
                  ...
                  placeholder={'请输入内容...(不超过' + news_content_max_length + '字)'}
              ...
              <Text style={{ fontSize: '15px', color: '#000000', marginRight: 10 }}>存储</Text>
              <Select
              ...
              {news_type != 'test2' && news_type != 'articles_1' && (
                <div style={{ margin: '10px 0' }}>
                  ...
                  />*/}
                  <span style={{ fontSize: '15px', color: '#000000' }}>权重</span>
                  <InputNumber
                    min={news_weights_value_min}
                    max={news_weights_value_max}
                    step={news_weights_value_step}
              ...
              {news_type != 'test2' && news_type != 'articles_1' && news_to_send_weight > 1 && (
                <div>
                ...
                {news_type === 'test2' || news_type === 'articles' ? (
                  <Button
                    ...
                    disabled={
                      ...
                      (news_content_to_send && news_content_to_send.length < 2) ||
                      news_content_to_send === '' 
                      //news_image_url === null
                      //|| (news_article_worth === '')
                      ...
                      <div>
                        ...
                        {item.weights >= news_weights_level_important ? (
                          <span
                            ...
                            {item.news_content}<br /> <img src={item.news_images[1]} width="50%" />
                          </span>
                        ) : (
                          <span
                            id="shareNewsListItemContent"
            ...
            <Modal
              ...
              <div>
                <NumericInput
                  ...
                  onFocus={this.onPaytipValueFocus}
                />
                <span style={{ fontSize: '15px', color: '#000000', marginLeft: 10 }}>{token.symbol}</span>
              </div>
              <div style={{ margin: '5px 0' }} />
              ...
              <TextArea
                ...
                //onFocus={this.onPaytipCommentFocus}
        ...
        {paytip_sending && (
          <Auth
            ...
            messages={{
              title: '打赏支付',
              scan: `打赏金额 ${tipValue} ${token.symbol}`,
              confirm: '在ABT钱包中确认',
              success: '支付成功!',
            }}
          />
