...
const {
  ...
  ImageCrop,
  ImageFileRemove,
  UserPaymentWebHdDirGet, //ad by ck
} = require('../libs/image');
...
async function NewsflashCreateAssetOnChain(fields) {
  ...
  //console.log('kkkimageUrl:', imageUrl)
  //console.log('kkk:', imageUrl.split(";",2)[0].split("/",2)[1])

  //add by ck
  var hd_suffix = '.jpg';
  var tmp = imageUrl.split(";",2)[0].split("/",2)[1];
  if (tmp === 'gif') {
    hd_suffix = '.gif'
  }
  const hd_filename = HashString('sha1', Date.now()+user.did)+hd_suffix;
  //const hd_filename = HashString('sha1', Date.now()+user.did)+'.jpg';
  const hd_file = UserPaymentHdDirGet(user.did)+'/'+hd_filename;
  //require('child_process').execSync('/usr/bin/mkdir -p ' + UserPaymentHdDirGet(user.did));
  const hd_web_file = UserPaymentWebHdDirGet(user.did)+'/'+hd_filename;
  imageUrl != '' && Base64ImageDataToFile(imageUrl, hd_file);

  //add by ck at 20200719 for img to ipfs
  //const hd_file_ipfs_hash = imageUrl != '' ? require('child_process').execSync('sudo /root/go/bin/ipfs add /home/forge/abtworld/src/' + hd_web_file).toString().split(' ')[1] : '';
  const hd_file_ipfs_hash = imageUrl != '' ? require('child_process').execSync('sudo /usr/local/bin/ipfs add /home/forge/abtworld/src/' + hd_web_file).toString().split(' ')[1] : '';
  //push to gateway.ipfs.io for backup
  var res_push = imageUrl != '' ? require('child_process').exec('curl -v https://gateway.ipfs.io/ipfs/' + hd_file_ipfs_hash + ' -s -o /dev/null') : '';

  //news text to local file by ck at 20200812
  var news_text = typeof fields.news_content != 'undefined' ? fields.news_content[0] : '';
  var text_filename = HashString('sha1', news_text);
  var text_file_dir = '/home/forge/abtworld/src/' + UserTextDirGet(user.did);
  var text_file_path = text_file_dir + '/' + text_filename;
  require('child_process').execSync('/usr/bin/mkdir -p ' + text_file_dir);
  require('child_process').execSync('echo -e "' + fields.news_content + '" > ' + text_file_path);

  //news text to local file to ipfs
  //var text_file_ipfs_hash = require('child_process').execSync('sudo /root/go/bin/ipfs add ' + text_file_path).toString().split(' ')[1];
  var text_file_ipfs_hash = require('child_process').execSync('sudo /usr/local/bin/ipfs add ' + text_file_path).toString().split(' ')[1];
  //push to gateway.ipfs.io for backup
  var res_text_file_push = require('child_process').exec('curl -v https://gateway.ipfs.io/ipfs/' + text_file_ipfs_hash + ' -s -o /dev/null');
  ...
  imageUrl != '' && Base64ImageDataToFile(imageUrl, UserPaymentBaseDirGet(user.did) + '/article_image.jpg');
  ...
  if (news_doc) {
    if (news_doc.state != 'commit') {
      ...
      news_doc.news_weights = fields.news_weights[0];
      news_doc.total_comment_minner_number = total_comment_minner_number;
      news_doc.total_like_minner_number = total_like_minner_number;
      news_doc.total_forward_minner_number = total_forward_minner_number;
      ...
      news_doc.news_images[0] = imageUrl != '' ? base64SmallImageData : '';
      news_doc.news_images[1] = imageUrl != '' ? hd_web_file : '';
      news_doc.news_images[2] = imageUrl != '' ? hd_file_ipfs_hash : '';
      news_doc.news_images[3] = text_file_ipfs_hash != '' ? text_file_ipfs_hash : '';
      await doc.save();
    }
  } else {
    /*save newsflash to db when not exist*/
    var news_images_arr = imageUrl != '' ? [base64SmallImageData, hd_web_file, hd_file_ipfs_hash, text_file_ipfs_hash] : ['', '', '', text_file_ipfs_hash];
    news_doc = new Newsflash({
      ...
      total_like_minner_number: total_like_minner_number,
      total_forward_minner_number: total_forward_minner_number,
      news_images: news_images_arr,
      //news_images: [base64SmallImageData, hd_web_file],
      state: 'commit',
      minner_state: 'idle',
      createdAt: Date(),
    });
  ...
  //注释掉以下代码，以便图文付费可用（newsflashset 接口不超时）
...
async function NewsflashItemAddComment(fields) {
  ...
  //if (user.did == 'z1Qqkbi1D385qSF5YNdvgp8DrVuuVL7VAVR') {
  //  return null;
  //}
...
async function getUserCommitNews(strAuthorDid){
  var new_docs = [];
  var found = 0;
  ...
  Newsflash.find().byAuthorDidAndState(strAuthorDid, 'commit').exec(function(err, docs){
    if(docs && docs.length>0){
      if(!isProduction){
        console.log('getUserCommitNews Found', docs.length, 'docs');
      }
      new_docs = docs;
    }else{
      if(!isProduction){
        console.log('getUserCommitNews doc not found!');
      }
    }
    found = 1;
  })
  ...
  if(!isProduction){
    console.log('getUserCommitNews wait counter', wait_counter);
  }
