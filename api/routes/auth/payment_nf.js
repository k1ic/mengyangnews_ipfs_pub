...
const { getNewsByAssetDid, getUserCommitNews, cleanUserDeadNews } = require('../newsflash');
...
async function paymentBackToUser(hash, forgeState, userDid) {
  var txRes = await waitAndGetTxHash(hash, env.chainId);
  if(txRes && txRes.getTx && txRes.getTx.code === 'OK' && txRes.getTx.info){
    ...
    if(!isProduction){
      console.log('Payment back to user', transferHash);
    }
  }else{
    if(!isProduction){
      console.log('paymentBackToUser hash=', hash, ' unavailable');
    }
  }
}

async function paymentHookForMinner(hash, asset_did, forgeState, userDid, newsHash) {
  var txRes = await waitAndGetTxHash(hash, env.chainId);
  ...
  if(txRes && txRes.getTx && txRes.getTx.code === 'OK' && txRes.getTx.info){
    ...
    if(tx_memo.module == 'newsflash'){
      ...
      if(typeof(asset_did) != "undefined" && asset_did.length > 0){
        var newsflash_doc = await getNewsByAssetDid(asset_did);
        if(newsflash_doc){
          ...
          newsflash_doc.news_hash = newsHash;
          newsflash_doc.news_time = tx_local_time;
          newsflash_doc.hash_href.push(newsflash_doc.data_chain_nodes[0].chain_host.replace('/api', '/node/explorer/txs/')+newsHash);
          ...
          if(ecoPartnerFromDb === true){
            ...
            for(var i=0;i<ecoPartnerList.length;i++){
              if(ecoPartnerList[i].state === 'active'){
                EcoPartnerDocs.push(ecoPartnerList[i]);
              }
            }
          }
              
          var eco_partner_ctatio = 0;
          if(EcoPartnerDocs && EcoPartnerDocs.length>0){
            EcoPartnerDocs.map(function( e ) {
              eco_partner_ctatio += e.cratio;
            });
          }
          if(eco_partner_ctatio > 0 && eco_partner_ctatio <= 1){
            for(var i=0;i<EcoPartnerDocs.length;i++){
              if(EcoPartnerDocs[i].cratio > 0){
                try {
                  var tmp_pay_value = forgeTxValueSecureConvert(pay_value_for_eco_partner*EcoPartnerDocs[i].cratio);
                  if(tmp_pay_value > 0){
                    transferHash = await ForgeSDK.sendTransferTx({
                      tx: {
                        itx: {
                          to: EcoPartnerDocs[i].did,
                    ...
                    if(!isProduction){
                      console.log('pay to eco partner did=', EcoPartnerDocs[i].did, 'value=', tmp_pay_value);
                    }
                  }else{
                    if(!isProduction){
                      console.log('Invalid pay value for eco partner did=', EcoPartnerDocs[i].did);
                    }
                  }
                } catch (err) {
                  transferHash = null;
                  if(!isProduction){
                    console.error('pay to eco partner did=', EcoPartnerDocs[i].did, 'err', err);
                  }
                }
              }
            }
          ...
          /*pay back to user when asset doc is null*/
          if(!isProduction){
            console.log('pay back to user because of asset doc is null');
          }
          await paymentBackToUser(hash, forgeState, userDid);
        }
      }else{
        /*pay back to user when invalid asset did in tx memo*/
        if(!isProduction){
          console.log('pay back to user because of invalid asset did in tx memo');
        }
        await paymentBackToUser(hash, forgeState, userDid);
      }       
    }else{
      /*pay back to user when invalid module in tx memo*/
      if(!isProduction){
        console.log('pay back to user because of invalid module in tx memo');
      }
      await paymentBackToUser(hash, forgeState, userDid);
    }
  }else{
    /*pay back to user when invalid tx hash*/
    if(!isProduction){
      console.log('pay back to user because of invalid tx hash');
    }
    await paymentBackToUser(hash, forgeState, userDid);
  }
...
async function newsflashPaymentHookV2(hash, forgeState, userDid) {
  try {
    ...
    newsflash_docs = await getUserCommitNews(userDid);
    if(newsflash_docs && newsflash_docs.length > 0){
      newsflash_doc = newsflash_docs[0];
      
      /*create asset*/
      transferHash = await createNewsflahAsset(newsflash_doc.content_did, newsflash_doc.data_chain_nodes[0].chain_id);
      /*Update newsflash doc*/
      if(transferHash && transferHash.length > 0){
        ...
        /*Minner and eco parter process*/
        var asset_did = newsflash_doc.asset_did;
        await paymentHookForMinner(hash, asset_did, forgeState, userDid, transferHash);
      }else{
        if(!isProduction){
          console.log('pay back to user because of create asset error');
        }
        await paymentBackToUser(hash, forgeState, userDid);
      }
      ...
      if(!isProduction){
        console.log('pay back to user because of empty commit news');
      }
      await paymentBackToUser(hash, forgeState, userDid);
    }
